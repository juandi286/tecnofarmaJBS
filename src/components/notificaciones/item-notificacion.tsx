...
export function ItemNotificacion({ notification, onMarkAsRead }: ItemNotificacionProps) {
  ...
  return (
    <Card className={cn("transition-all duration-300 ease-in-out hover:shadow-md", cardBorderClass, notification.read ? 'bg-muted/30' : 'bg-card')}>
      <CardHeader className="flex flex-row items-start gap-3 space-y-0 p-4">
        ...
        {!notification.read && (
            <Badge variant="default" className="bg-accent text-accent-foreground text-xs">Nueva</Badge>
        )}
      </CardHeader>
